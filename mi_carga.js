...
function openSolicitudModal(fecha, bloque) {
    ...
    const userRequestInCell = solicitudesEnCelda.find(
        item => item.Funcionario_idFuncionario == currentLoggedInUserId
    );
    ...
    if (userRequestInCell) {
        ...
        // if(mensajeExistenteSpan) mensajeExistenteSpan.textContent = '';
         // Asegurarse de que el formulario está visible si estaba oculto
         // formularioSolicitud.style.display = 'block';
    }
                    ...
                    if (j === 0) {
                        ...
                        if (solicitudesEnCelda.length > 0) {
                            ...
                            cell.addEventListener('click', function() {
                                ...
                                openSolicitudModal(fecha_celda, bloque_celda);
                            });
                        }
...
if (btnSolicitarOtro) {
    btnSolicitarOtro.addEventListener('click', function() {
        ...
        openSolicitudModal(fechaSolicitada, bloqueSolicitado);
    });
}
